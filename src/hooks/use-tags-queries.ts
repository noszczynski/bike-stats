...
export function useCreateTag() {
  ...
  return useMutation({
    mutationFn: async (tagData: Omit<Tag, 'id' | 'created_at' | 'updated_at'>): Promise<Tag> => {
      const response = await fetch('/api/tags', {
        method: 'POST',
        headers: {
...
export function useAddTagToActivity() {
  ...
  return useMutation({
    mutationFn: async ({ activityId, tagId }: { activityId: string; tagId: string }): Promise<ActivityTag> => {
      const response = await fetch(`/api/trainings/${activityId}/tags`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
export function useRemoveTagFromActivity() {
  ...
  return useMutation({
    mutationFn: async ({ activityId, tagId }: { activityId: string; tagId: string }): Promise<void> => {
