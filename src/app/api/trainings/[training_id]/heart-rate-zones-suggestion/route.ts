...
export function calculateHeartRateZonesByTrackpoints(
    trackpoints: Prisma.TrackpointGetPayload<{
        select: {
            heart_rate_bpm: true;
            timestamp: true;
            speed_ms: true;
    ...
    const sortedTrackpoints = trackpoints.sort(
    ...
    const isLikelyStop = (current: any, next: any, timeDiff: number) => {
        ...
        if (
            currentSpeed < STOP_DETECTION_CONFIG.STOP_SPEED_THRESHOLD_MS &&
            nextSpeed < STOP_DETECTION_CONFIG.STOP_SPEED_THRESHOLD_MS
        )
            return true;
        ...
        if (
            hrDiff > STOP_DETECTION_CONFIG.HR_DROP_THRESHOLD_BPM &&
            timeDiff > STOP_DETECTION_CONFIG.HR_STABILITY_TIME_THRESHOLD_SECONDS
        )
            return true;
    ...
    for (let i = 0; i < sortedTrackpoints.length; i++) {
        ...
        let nextHRIndex = i + 1;
        while (
            nextHRIndex < sortedTrackpoints.length &&
            (sortedTrackpoints[nextHRIndex].heart_rate_bpm == null ||
                (sortedTrackpoints[nextHRIndex].speed_ms || 0) <
                    STOP_DETECTION_CONFIG.MIN_SPEED_FOR_HR_ZONES_MS)
        ) {
            nextHRIndex++;
        }
        ...
        if (nextHRIndex < sortedTrackpoints.length) {
            ...
            if (isLikelyStop(current, nextHR, timeDiff)) {
                ...
                duration = Math.max(
                    STOP_DETECTION_CONFIG.DEFAULT_MEASUREMENT_DURATION_SECONDS,
                    timeDiff,
                );
            }
        } else {
            ...
            while (
                ...
                (sortedTrackpoints[prevHRIndex].heart_rate_bpm == null ||
                    (sortedTrackpoints[prevHRIndex].speed_ms || 0) <
                        STOP_DETECTION_CONFIG.MIN_SPEED_FOR_HR_ZONES_MS)
            ) {
                prevHRIndex--;
            }
    ...
    return {
        zone_1: {
            time: formatTime(zoneTimes.zone_1),
            percentage: round((zoneTimes.zone_1 / totalHRTime) * 100, 1),
        },
        zone_2: {
            time: formatTime(zoneTimes.zone_2),
            percentage: round((zoneTimes.zone_2 / totalHRTime) * 100, 1),
        },
        zone_3: {
            time: formatTime(zoneTimes.zone_3),
            percentage: round((zoneTimes.zone_3 / totalHRTime) * 100, 1),
        },
        zone_4: {
            time: formatTime(zoneTimes.zone_4),
            percentage: round((zoneTimes.zone_4 / totalHRTime) * 100, 1),
        },
        zone_5: {
            time: formatTime(zoneTimes.zone_5),
            percentage: round((zoneTimes.zone_5 / totalHRTime) * 100, 1),
        },
    };
}
