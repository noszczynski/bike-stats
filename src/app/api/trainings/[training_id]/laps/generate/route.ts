...
export async function POST(
  ...
  try {
    ...
    const trackpointsWithDistance = trackpoints.filter(tp => tp.distance_m !== null && tp.distance_m !== undefined);
    ...
    const calculateLapStats = (lapTrackpoints: typeof trackpoints) => {
      ...
      // Calculate duration
      const lapDuration = (new Date(lapEnd.timestamp).getTime() - new Date(lapStart.timestamp).getTime()) / 1000;
      ...
      const speedValues = lapTrackpoints.filter(tp => tp.speed_ms !== null).map(tp => tp.speed_ms!);
      ...
      const hrValues = lapTrackpoints.filter(tp => tp.heart_rate_bpm !== null).map(tp => tp.heart_rate_bpm!);
      ...
      const cadenceValues = lapTrackpoints.filter(tp => tp.cadence_rpm !== null).map(tp => tp.cadence_rpm!);
      const avgCadence = cadenceValues.length > 0 ? Math.round(cadenceValues.reduce((a, b) => a + b, 0) / cadenceValues.length) : null;
      const maxCadence = cadenceValues.length > 0 ? Math.max(...cadenceValues) : null;

      // Calculate elevation gain
      const altitudeValues = lapTrackpoints.filter(tp => tp.altitude_m !== null).map(tp => tp.altitude_m!);
      let elevationGain = 0;
    ...
    for (let i = 1; i < trackpoints.length; i++) {
      ...
      if (lapDistance >= targetDistanceM) {
        console.log(`Creating lap ${currentLapNumber}: lapDistance=${lapDistance}, targetDistance=${targetDistanceM}`);
    ...
    if (lapStartIndex < trackpoints.length - 1) {
      ...
      const remainingDistance = (trackpoints[trackpoints.length - 1].distance_m || 0) - lapStartDistance;
