...
export const AUTO_TAGGING_RULES: TagRule[] = [
    ...
    },
    {
        ...
        icon: "heart",
        condition: data =>
            (data.avg_heart_rate_bpm || 0) < 135 && (data.avg_heart_rate_bpm || 0) > 0,
    },
    {
        tagName: "Speed Demon",
    ...
    },
    {
        ...
        icon: "leaf",
        condition: data =>
            (data.avg_heart_rate_bpm || 0) < 130 && (data.avg_heart_rate_bpm || 0) > 0,
    },
];
...
export async function getActivityAnalysis(activityId: string): Promise<ActivityAnalysis | null> {
    ...
    const totalElevationGain = activity.laps.reduce(
        (sum: number, lap) => sum + (lap.total_elevation_gain_m || 0),
        0,
    );

    const avgSpeed =
        activity.laps.length > 0
            ? activity.laps.reduce((sum: number, lap) => sum + (lap.avg_speed_ms || 0), 0) /
    ...
    const avgHeartRate =
        activity.laps.length > 0
            ? activity.laps
                  .filter(lap => lap.avg_heart_rate_bpm)
                  .reduce((sum: number, lap) => sum + (lap.avg_heart_rate_bpm || 0), 0) /
    ...
    return {
        ...
        type: activity.type,
        distance_m:
            totalDistance > 0 ? totalDistance : activity.strava_activity?.distance_m || undefined,
        elevation_gain_m:
            totalElevationGain > 0
                ? totalElevationGain
                : activity.strava_activity?.elevation_gain_m || undefined,
        moving_time_s:
            totalMovingTime > 0
                ? totalMovingTime
                : activity.strava_activity?.moving_time_s || undefined,
        avg_speed_ms: avgSpeed,
        max_speed_ms: maxSpeed,
        avg_heart_rate_bpm:
            avgHeartRate || activity.strava_activity?.avg_heart_rate_bpm || undefined,
        max_heart_rate_bpm:
            maxHeartRate || activity.strava_activity?.max_heart_rate_bpm || undefined,
...
export async function applyAutoTagging(activityId: string): Promise<void> {
    ...
    console.log(
        ...
        applicableRules.map(r => r.tagName),
    );
