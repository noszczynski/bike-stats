...
export const calculateTrainingLoad = (
    ...
    const heartRateScore = training.avg_heart_rate_bpm
        ? Math.min(training.avg_heart_rate_bpm / maxHR, 1)
        : 0.5;
    ...
    const intensityScore = Math.round(
        (distanceScore * 0.3 + speedScore * 0.3 + heartRateScore * 0.2 + elevationScore * 0.2) *
            100,
    );
