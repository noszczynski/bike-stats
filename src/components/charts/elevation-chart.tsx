...
export function ElevationChart({ trainings }: { trainings: Training[] }) {
    ...
    const data = sortedTrainings.map((training, index) => {
        ...
        const avgElevation =
            trainingsToAverage.reduce((sum, t) => sum + t.elevation_gain_m, 0) / trainingsToAverage.length;
        ...
        <Card>
                                ...
                                return (
                                    <ChartTooltipContent
                                        ...
                                        payload={payload.map((p) => {
                                            const key = p.name === 'elevation' ? 'elevation' : 'elevationPerKm';
            ...
            <CardFooter className='flex-col items-start gap-2 text-sm'>
                <div className='flex gap-2 leading-none font-medium'>
                    {Math.abs(elevationProgress).toFixed(1)}% {elevationProgress > 0 ? 'wzrost' : 'spadek'} średniego
                    przewyższenia od pierwszego treningu <TrendIcon className='h-4 w-4' />
                </div>
                <div className='text-muted-foreground leading-none'>
                    Pokazuje średnie przewyższenie i średnie przewyższenie na kilometr dla wszystkich treningów do
                    danego momentu
                </div>
            </CardFooter>
        </Card>
