...
export function IntensityChart({ trainings }: { trainings: Training[] }) {
    ...
    const data = sortedTrainings.map((training, index) => {
        ...
        const intensities = trainingsToAverage.map(t => {
            ...
            const heartRateScore = t.avg_heart_rate_bpm
                ? (t.avg_heart_rate_bpm / maxHeartRate) * 20
                : 0;
        ...
        const avgIntensity =
            intensities.reduce((sum, score) => sum + score, 0) / intensities.length;
        ...
        <Card>
            ...
            <CardFooter className="flex-col items-start gap-2 text-sm">
                <div className="flex gap-2 leading-none font-medium">
                    {Math.abs(intensityProgress).toFixed(1)}%{" "}
                    {intensityProgress > 0 ? "wzrost" : "spadek"} średniej intensywności od
                    pierwszego treningu <TrendIcon className="h-4 w-4" />
                </div>
                <div className="text-muted-foreground leading-none">
                    Pokazuje średnią intensywność treningów obliczoną na podstawie dystansu,
                    prędkości, tętna i przewyższenia
                </div>
            </CardFooter>
        </Card>
    );
