...
export function HeartRateTimeChart({ trainings }: { trainings: Training[] }) {
    ...
    const data = sortedTrainings.map((training, index) => {
        ...
        const avgHeartRateMovingAvg =
            trainingsToAverage.reduce((sum, t) => sum + (t.avg_heart_rate_bpm || 0), 0) / trainingsToAverage.length;

        // Calculate moving average for max heart rate
        const maxHeartRateMovingAvg =
            trainingsToAverage.reduce((sum, t) => sum + (t.max_heart_rate_bpm || 0), 0) / trainingsToAverage.length;
                ...
                <ChartContainer config={chartConfig} className='aspect-auto h-80'>
                    <AreaChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                        ...
                        <YAxis
                            yAxisId='right'
                            orientation='right'
                            label={{ value: 'BPM (maksymalne)', angle: 90, position: 'insideRight' }}
                            tickLine={false}
                            axisLine={false}
                            domain={['dataMin - 5', 'dataMax + 5']}
                        />
                                ...
                                return (
                                    <ChartTooltipContent
                                        ...
                                        payload={payload.map((p) => {
                                            const key = p.name === 'avgHeartRate' ? 'avgHeartRate' : 'maxHeartRate';
                                            const rawKey =
                                                p.name === 'avgHeartRate' ? 'rawAvgHeartRate' : 'rawMaxHeartRate';
