...
export function HeartRateChart({ trainings }: { trainings: Training[] }) {
    ...
    const data = sortedTrainings.map((training, index) => {
        ...
        trainingsToAverage.forEach(t => {
            if (t.heart_rate_zones) {
                ...
                const totalMinutes = Object.values(zones).reduce(
                    (sum, time) => sum + parseTimeToMinutes(time),
                    0,
                );
    ...
    const heartRateTrend =
        firstHeartRate > 0 ? ((lastHeartRate - firstHeartRate) / firstHeartRate) * 100 : 0;
        ...
        <Card>
            ...
            <CardContent>
                <ChartContainer config={chartConfig} className="aspect-auto h-80">
                    ...
                    >
                        ...
                        <ChartTooltip
                            cursor={false}
                            content={<ChartTooltipContent formatter={formatToPercentage} />}
                        />
                        <Area
                            ...
                            content={({ payload }) => {
                                if (payload && payload.length) {
                                    const legendPayload = payload.map(entry => ({
                                        ...entry,
                                        value: entry.value,
                                        name: chartConfig[entry.dataKey as keyof typeof chartConfig]
                                            .label,
                                        color: chartConfig[
            ...
            <CardFooter className="flex-col items-start gap-2 text-sm">
                ...
                <div className="text-muted-foreground leading-none">
                    Pokazuje średni procentowy rozkład czasu w strefach tętna dla wszystkich
                    treningów do danego momentu
                </div>
            </CardFooter>
        </Card>
    );
