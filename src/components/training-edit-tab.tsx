...
import { useLaps, useFitStatus } from '@/hooks/use-laps-queries';
import { Training } from '@/types/training';
import { Lap } from '@/types/lap';
import { cn } from '@/lib/utils';
import { Activity, Clock, Zap, Heart, MapPin } from 'lucide-react';
                                    ...
                                    {Array.from({ length: 10 }, (_, i) => i + 1).map((value) => (
                                        <div key={value} className="flex items-center justify-center">
                                            ...
                                            <Label
                                                ...
                                                className={cn(
                                                    'flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border-2 text-sm font-medium transition-all hover:bg-accent peer-checked:bg-primary peer-checked:text-primary-foreground peer-checked:border-primary',
                                                    {
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-4">
                                    <Button
                                        ...
                                        disabled={!hasTrackpoints || generateLapsMutation.isPending}
                                        variant="outline"
                                    >
                                        {generateLapsMutation.isPending ? 'Generowanie...' : 'Generuj odcinki'}
                                    </Button>
                                    {!hasTrackpoints && (
                                        <Alert>
                                            <AlertDescription>
                                                Brak danych trackpoints. Wgraj plik FIT, aby wygenerować odcinki.
                                            </AlertDescription>
                                        </Alert>
                                    )}
                                </div>
                                <div className="text-sm text-muted-foreground">
                                    Odcinki będą generowane co 5km. Ostatni odcinek będzie zawierał pozostałą odległość (bez względu na długość).
                                </div>
                            </div>
                            ...
                            {lapsLoading ? (
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-sm text-muted-foreground mt-2">Ładowanie odcinków...</p>
                                </div>
                            ) : laps.length > 0 ? (
                                <div className="space-y-4">
                                    <h4 className="font-medium">Odcinki treningu ({laps.length})</h4>
                                            ...
                                            return (
                                                    ...
                                                    <div className="flex items-start justify-between">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-sm font-medium">
                                                                {formatDistance(lap.distance_m)} km
                                                            </span>
                                                        </div>
                                                ...
                                                <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        <span>Śr. prędkość: {formatSpeed(lap.avg_speed_ms)} km/h</span>
                                                    </div>
                                                    <div className="flex items-center gap-1">
                                                        <Zap className="h-4 w-4 text-muted-foreground" />
                                                        <span>Maks. prędkość: {formatSpeed(lap.max_speed_ms)} km/h</span>
                                                    </div>
                                                    <div className="flex items-center gap-1">
                                                        <Heart className="h-4 w-4 text-muted-foreground" />
                                                        <span>Śr. HR: {lap.avg_heart_rate_bpm || 'N/A'} bpm</span>
                                                    </div>
                                                    <div className="flex items-center gap-1">
                                                        <Clock className="h-4 w-4 text-muted-foreground" />
                                                        <span>Maks. HR: {lap.max_heart_rate_bpm || 'N/A'} bpm</span>
                                                    </div>
                                                </div>
                                            </Card>
                                            );
                                ...
                                <div className="text-center py-8 text-muted-foreground">
                                    ...
                                    <p className="text-sm mt-1">
                                        Wgraj plik FIT lub wygeneruj odcinki, aby zobaczyć szczegóły
                                    </p>
                                </div>
                            )}
                        </div>
