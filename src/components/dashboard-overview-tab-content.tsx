...
export async function DashboardOverviewTabContent() {
    ...
    const getTrendIcon = (trend: number, positiveIsBetter = true) => {
        ...
        const isImprovement =
            (positiveIsBetter && isPositive) || (!positiveIsBetter && !isPositive);
    ...
    const recentTrainings = trainings.filter(training =>
        date(training.date).isAfter(thirtyDaysAgo),
    );
    ...
    const recentAvgSpeed =
        recentTrainings.length > 0
            ...
            : 0;
    const previousAvgSpeed =
        previousTrainings.length > 0
            ? previousTrainings.reduce((acc, t) => acc + t.avg_speed_kmh, 0) /
